...
const StoryboardInfo = ({ data }: props) => {
  ...
  return (
    <InfoContainer>
      ...
      <GridContainer>
        <GridItem bgColor={"lightgray"} fontSize={"large"}>Scene</GridItem>
        <GridItem>{data.locate}</GridItem>
        <GridItem>{data.time}</GridItem>
        <GridItem bgColor={"lightgray"} fontSize={"large"}>Cut</GridItem>
        <GridItem bgColor={"lightgray"} fontSize={"large"}>#00{data.sceneNum}</GridItem>
...
const GridItem = styled.div<{ rowSpan?: number; colSpan?: number, bgColor?: string, fontSize?: string }>`
  text-align: center;
